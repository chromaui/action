...
async function runChromatic(options) {
    ...
    const packagePath = await pkg_up_1.default(); // the user's own package.json
    const packageJson = await jsonfile_1.readFile(packagePath);
    const context = Object.assign(Object.assign({}, parseArgs_1.default([])), { packagePath, packageJson, env, log, sessionId, flags: options });
...
async function run() {
    ...
    try {
        ...
        const skip = core_1.getInput('skip');
